...
library(patchwork)
library(sessioninfo)
devtools::install_github("lahuuki/DeconvoBuddies")
...
#sce <- list(all = sce.all.n12)

#sceXct <- cross2(sce,ct)
#names(sceXct) <- map(cross2(names(sce),names(ct)), ~paste0(.x[[1]],"_",.x[[2]]))

test <- get_mean_ratio2(sce.all.n12[1:100,], "cellType.Broad")
mean_ratio <- map(ct, ~get_mean_ratio2(sce = sce.all.n12, cellType_col = .x))
map_int(mean_ratio, nrow)
#   broad specific
#   17462    75663

#### Run findMarkers ####
markers.t.1vAll <- map(ct, ~findMarkers_1vAll(sce = sce.all.n12, cellType_col = .x))
