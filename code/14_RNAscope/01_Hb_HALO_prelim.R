...
gg_copies <- copies_tab |> select(-sum) |> ggpairs()
ggsave(gg_copies, filename = here(plot_dir, "gg_copies.png"), height = 9, width = 9)
...
halo_copies_long <- halo_prelim |>
  select(XMax, YMax, ends_with("Copies")) |>
  pivot_longer(!c(XMax, YMax), names_to = "probe", values_to = "copies")

hex_copies_facet <- ggplot(halo_copies_long) +
