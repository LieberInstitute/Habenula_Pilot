...
## set up 
plot_dir <- here("plots", "14_RNAscope", "03_LHb_HALO")
if(!dir.exists(plot_dir)) dir.create(plot_dir, recursive = TRUE)

## load data
fn <- list(prelim = here("processed-data", "14_RNAscope", "HALO_data", 
                          "LHbExperiment_Br6462_secondpass.csv")
            )
...
# 3 570 CHAT (LHb.2) job1862 16527   685   229   290   353
# 4 570 CHAT (LHb.2) job1864 10264   741   250   338   401
# 5 620 EBF3 (LHb.3) job1862 16343  1026    NA   298   417
# 6 620 EBF3 (LHb.3) job1864 10372   842    NA   441   339
# 7 690 CCK (LHb.1)  job1862 17710   161    51    78    84
# 8 690 CCK (LHb.1)  job1864 11475   235    62   102   120
...
ggsave(cell_count_quant, filename = here(plot_dir, paste0("LHb_cell_count_quant_facet.png")), height = 7, width = 9)
ggsave(cell_count_quant, filename = here(plot_dir, paste0("LHb_cell_count_quant_facet.pdf")), height = 7, width = 9)
...
ggsave(copies_density, filename = here(plot_dir, "LHbcopies_denisty.png"), height = 5)
...
ggsave(cell_max_quant_plot, filename = here(plot_dir, paste0("LHb_cell_max_quant.png")), height = 9, width = 9)
ggsave(cell_max_quant_plot, filename = here(plot_dir, paste0("LHb_cell_max_quant.pdf")), height = 9, width = 9)
...
ggsave(gg_copies, filename = here(plot_dir, "LHb_gg_copies.png"), height = 12, width = 12)
...
ggsave(gg_copies_hex, filename = here(plot_dir, "LHb_gg_copies_hex.png"), height = 12, width = 12)


## heatmap grid
halo_copies_long3 <- halo_copies_long2 |> 
  filter(job == "job1862") |>
  select(`Object Id`, XMin, XMax, YMin, YMax, probe, probe2, copies, copy_quant) |>
  separate(probe2, into = c("wl", "marker", "cluster"), extra = "merge")

halo_copies_wide <- halo_copies_long3 |>
  select(`Object Id`, marker, copies) |>
  pivot_wider(names_from = "marker", values_from = "copies")


expand_grid(m1 = experiment$marker,
            m2 = experiment$marker)

marker_comb <- t(combn(experiment$marker,2))
colnames(marker_comb) <- c("m1", "m2")

copy_tile <- halo_copies_wide |>
  count(POU4F1, CHAT) |>
  ggplot(aes(x = POU4F1, y = CHAT, fill = n)) +
  scale_fill_continuous(type = "viridis", limits = c(1,200), "n\n(max 100)") +
  geom_tile()

ggsave(copy_tile, filename = here(plot_dir, "copy_tile.png"))
