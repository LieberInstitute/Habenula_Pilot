...
## TSNE Habenula only - for review

sce_hb <- sce[,grepl("Hb", sce$final_Annotations)]

pdf(here(plot_dir, "TSNE_by_CellType_HB.pdf"),
    width = 6, height = 5)

plotReducedDim(sce_hb, dimred = "TSNE") +
    geom_point(aes(color = sce_hb$final_Annotations)) +
    guides(color = guide_legend(title="Hb Cell Type")) +
    scale_colour_manual(values = sn_colors) +
    labs(x = "TSNE Dimension 1", y = "TSNE Dimension 2")
