...
def load_data(
        plink_prefix_path, expression_bed, covariates_file, add_chr = False,
        fix_geno_names = False
    ):
    ...
    if fix_geno_names:
        ...
        fam = pd.read_table(
            plink_prefix_path + '.fam',
            delimiter = " ",
            names = ["V" + str(i) for i in range(6)]
        )
        genoSamples = [
            str(v1) + "_" + str(v0) for (v0,v1) in zip(fam.V1, fam.V0)
        ]
...
covar = str(in_dir / "covariates.txt")
expres = str(in_dir / "logcounts.bed.gz")

genotype_df, variant_df, phenotype_df, phenotype_pos_df, covariates_df = load_data(
    plink_prefix, expres, covar, add_chr = False, fix_geno_names = False
)
